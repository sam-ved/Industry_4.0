...
def load_feature_scaler():
    ...
    if os.path.exists(model_path):
        try:
            with open(model_path, "rb") as f:
                _loaded_models["scaler"] = pickle.load(f)
...
def load_label_encoders():
    ...
    encoders = {}
    encoder_names = ["le_failure", "le_machine", "le_machine_type", "le_maintenance"]
    ...
    try:
        ...
        for key, encoder in label_encoders.items():
            ...
            if categorical_key in processed:
                if encoder:
                    processed[categorical_key] = encoder.transform([processed[categorical_key]])[0]
                else:
        ...
        if scaler:
            numerical_keys = [k for k in processed.keys() if k not in ["failure_type", "machine", "machine_type", "maintenance_type"]]
